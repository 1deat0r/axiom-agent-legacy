...
Release docs use the Axiom package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
TypeBox exports are re-exported from `axiom-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'axiom-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'axiom-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'axiom-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'axiom-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'axiom-ai';
...
```typescript
import { getModel, stream } from 'axiom-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'axiom-ai';
...
```typescript
import { Model, stream } from 'axiom-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Axiom thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'axiom-ai';
...
```typescript
import { getModel, complete, Context } from 'axiom-ai';
...
```typescript
import { Context, getModel, complete } from 'axiom-ai';
...
- OAuth login flows are not supported in browser environments. Use the `axiom-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'axiom-ai';
...
```bash
npx axiom-ai login              # interactive provider selection
npx axiom-ai login anthropic    # login to specific provider
npx axiom-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `axiom-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from 'axiom-ai/oauth';
...
```typescript
import { getModel, complete } from 'axiom-ai';
import { getOAuthApiKey } from 'axiom-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `axiom-ai`
