...
Release docs use the Axiom package names. The source workspace manifests still keep inherited package names until the namespace migration is complete.
...
```typescript
import { Agent } from "axiom-core";
import { getModel } from "axiom-ai";
...
```typescript
import { Agent, streamProxy } from "axiom-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "axiom-core";
