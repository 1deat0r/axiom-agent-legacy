> Axiom can create TUI components. Ask it to build one for your use case.
...
Axiom shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while Axiom is streaming a response.
...
// Restore Axiom's default spinner
ctx.ui.setWorkingIndicator();
```
