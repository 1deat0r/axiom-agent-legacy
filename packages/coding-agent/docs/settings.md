...
| `.axiom/agent/settings.json` | Project (current directory) |
...
Stable builds fetch the release manifest at `https://pub-728493de92a943e2a9b2d17b4719f318.r2.dev/latest.json`. Beta builds fetch `beta.json` and continue following beta updates. Override the base URL with `AXIOM_DOWNLOAD_BASE_URL`.
...
```json
{
  "version": "0.73.1",
  "package": "axiom",
  "tarball": "releases/v0.73.1/axiom-0.73.1.tgz"
}
```

`version` is required. `package` is optional and may also be named `packageName`; it defaults to the current package name. `tarball` is optional; when present, Axiom installs that tarball instead of the package name. Relative tarball paths resolve against `AXIOM_DOWNLOAD_BASE_URL`.
...
```bash
AXIOM_TELEMETRY=0 axiom
DO_NOT_TRACK=1 axiom
axiom --offline
```

`AXIOM_TELEMETRY_ENDPOINT` overrides the ingestion endpoint for development and self-hosted deployments.
...
`idleEvictionMinutes` is a global daemon policy and is read only from `~/.axiom/agent/settings.json`. Set it to a positive number to configure the idle threshold.
...
```json
{ "sessionDir": ".axiom/agent/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `AXIOM_SESSION_DIR`, the legacy `AXIOM_CODING_AGENT_SESSION_DIR`, then `sessionDir` in `settings.json`.
...
Paths in `~/.axiom/agent/settings.json` resolve relative to `~/.axiom/agent`. Paths in `.axiom/agent/settings.json` resolve relative to `.axiom/agent`. Absolute paths and `~` are supported.
...
Project settings (`.axiom/agent/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.axiom/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .axiom/agent/settings.json (project)
{
