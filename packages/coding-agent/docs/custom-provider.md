...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. Axiom waits for the factory before startup continues, so the provider is available during interactive startup and to `axiom model list`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map Axiom levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
interface ProviderModelConfig {
  ...
  /** Maps Axiom thinking levels to provider/model-specific values; null marks a level unsupported. */
