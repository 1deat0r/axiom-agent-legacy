...
Axiom saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.
...
axiom --resume [path|id]  # Browse past sessions or resume one directly
axiom --no-session        # Ephemeral mode; do not save
axiom --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `axiom --resume` opens the same picker at startup, and `axiom --resume <path|id>` resumes a specific session.

An invalid ID exits with the closest unambiguous session ID when one is available. To open the picker and send an initial prompt after selecting a session, separate the prompt with `--`: `axiom --resume -- "continue this work"`.
...
When available, Axiom uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `axiom --resume`.
...
When `/tree` switches away from one branch to another, Axiom can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
