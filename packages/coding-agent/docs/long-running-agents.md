...
Axiom combines daemon-backed session workers with persistent state, scheduled prompts, direct agent messaging, goals, and bounded autonomous continuations. These features serve different purposes but share the same session and worker runtime.
...
```bash
axiom agents                  # Open the agents view
axiom rename <agent> <name>   # Give an agent a stable readable name
axiom stop <agent>            # Stop one agent
axiom status                  # Inspect background services
axiom doctor [--fix]          # Diagnose or repair service state
axiom shutdown [--force]      # Stop all agents and services
```
...
```bash
axiom send <agent> "Please verify the latest migration"
```
...
Axiom has three related scheduling surfaces:
...
| `axiom schedule` | User or automation | General one-time or cron prompts targeted at an agent. |
...
```bash
axiom schedule add worker "in 30m" -- "Check the benchmark result"
axiom schedule add worker "0 9 * * 1-5" -- "Review open work"
axiom schedule list --all
...
Autonomous mode is a bounded host policy for runs where no human input is expected. Axiom adds follow-up continuations until configured quality gates pass or a continuation, turn, token, or wall-clock limit is reached.
...
Autonomous mode supports limits for continuations, assistant turns, tokens, and wall-clock duration. Gate commands run before the session may finish; a failed gate returns its bounded output to the agent for another attempt. Axiom avoids rerunning the same failed gate when the workspace has not changed.
...
Automatic compaction handles context growth during long tasks. On overflow or near the configured threshold, Axiom summarizes older messages, retains recent context, and continues. The IPython kernel persists through compaction, so variables, imports, helper functions, and task state remain available.
