...
```bash
axiom --mode json "Your prompt"
```

Outputs all session events as JSON lines to stdout. Useful for integrating Axiom into other tools or custom UIs.
...
```bash
axiom --mode json "List files" 2>/dev/null | jq -c 'select(.type == "message_end")'
```
