...
Axiom uses the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) for reliable modifier key detection. Most modern terminals support this protocol, but some require configuration.
...
That mapping sends a raw linefeed byte. Inside Axiom, that is indistinguishable from `Ctrl+J`, so tmux and Axiom no longer see a real `shift+enter` key event.
...
Add to `settings.json` (Ctrl+Shift+, or Settings → Open JSON file) to forward the modified Enter keys Axiom uses:
...
- Windows Terminal binds `Alt+Enter` to fullscreen by default. That prevents Axiom from receiving `Alt+Enter` for follow-up queueing.
- Remapping `Alt+Enter` to `sendInput` forwards the real key chord to Axiom instead.
...
If you want the hardware cursor visible, set `PI_HARDWARE_CURSOR=1` before running `axiom` (disabled by default for compatibility).
