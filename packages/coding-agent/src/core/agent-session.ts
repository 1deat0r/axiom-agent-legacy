...
export class AgentSession {
	...
	private async _applyRefine(
		...
		try {
			...
			this.sessionManager.appendCustomEntry("axiom.refinement", result);
			this._baseSystemPrompt = this._rebuildSystemPrompt(this.getActiveToolNames());
			this.agent.state.systemPrompt = this._baseSystemPrompt;
			try {
	...
	private _addWebsearchKeyEnv(env: Record<string, string>): void {
		if (this._agentDir) {
			env.AXIOM_CODING_AGENT_DIR = this._agentDir;
		}
	...
	private _createEphemeralRlmSessionDir(): string {
		this._rlmSessionDir = mkdtempSync(join(tmpdir(), "axiom-rlm-"));
		return this._rlmSessionDir;
	}
