	...
	switch (command) {
		...
		case "update": {
			...
			if (hasLegacySelfTarget && hasLegacyPackageTarget) {
				return fail(
					"Axiom and package updates are now separate.",
...
function rejectRemovedCommand(args: string[]): PublicCommandResult {
	...
	if (command === "daemon") {
		replacement = 'Run "axiom help" to see the agent commands.';
	} else if (command === "app" && subcommand === "update") {
		replacement = 'Use "axiom update".';
	} else if (command === "install") {
		replacement = 'Use "axiom package install".';
	} else if (command === "remove" || command === "uninstall") {
		replacement = 'Use "axiom package remove".';
	} else if (command === "manage") {
		replacement = 'Use "axiom agents".';
	}
...
async function runPackage(args: string[]): Promise<PublicCommandResult> {
	...
	if (subcommand === "update") {
		if (
			...
			return fail('Package updates accept only an optional source. Use "axiom update --force" to update Axiom.');
		}
		if (rest.length > 1) {
			return fail(`Usage: ${APP_NAME} package update [source]`);
		}
		if (rest[0] && isSelfUpdateSource(rest[0])) {
			return fail('Use "axiom update" to update Axiom.');
		}
