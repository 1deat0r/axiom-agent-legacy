...
const OWNED_WORKER_ENV = "AXIOM_INTERNAL_OWNED_WORKER";
const OWNED_RECOVERY_DESCRIPTOR_ENV = "AXIOM_INTERNAL_OWNED_RECOVERY_DESCRIPTOR";
const OWNED_PROFILE_ENV = "AXIOM_INTERNAL_OWNED_PROFILE";
...
export async function runOwnedSessionWorkerFrontend(
	...
	const recoveryDescriptorPath = join(tmpdir(), `axiom-owned-${process.pid}-${randomUUID().slice(0, 12)}.json`);
	...
	const anonymousRpcIdPrefix = `axiom-owned-${randomUUID()}`;
...
export async function maybeRunOwnedSessionWorkerFrontend(
	...
	if (!forceLegacyFrontend && process.env.AXIOM_INTERNAL_LEGACY_OWNED_WORKER_FRONTEND !== "1") {
		return false;
	}
