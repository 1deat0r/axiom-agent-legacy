...
export class StaleDaemonError extends Error {
	constructor(
		...
		super(
			`An incompatible Axiom daemon is running.\n\n${daemonIdentity}\n` +
...
async function ensureDaemonRunning(socketPath: string, spawnCwd?: string): Promise<void> {
	...
	const throwIfFailed = () => {
		...
		if (childFailure.type === "error") {
			throw new Error(`Failed to spawn Axiom daemon: ${childFailure.error.message}.${logTail}`);
		}
		const signal = childFailure.signal ? `, signal ${childFailure.signal}` : "";
		throw new Error(
			`Axiom daemon exited during startup (code ${childFailure.code ?? "unknown"}${signal}).${logTail}`,
		);
	};
