...
/**
 * Translate axiom session events into ACP `session/update` payloads.
 *
 * Kept as a pure function so the mapping is testable without a live ACP client
 * or a running agent. Returning an array lets one axiom event fan out to
...
/** axiom's model-facing tool is IPython; bash is the secondary escape hatch. */
...
export function acpUpdatesForSessionEvent(
	...
	switch (event.type) {
		...
		// axiom concepts with no ACP counterpart. They are still part of a
...
const BASH_TOOL_CALL_PREFIX = "axiom-bash";
