...
export const AXIOM_BUILD_ID_ENV = "AXIOM_BUILD_ID";
export const AXIOM_LAUNCHER_PATH_ENV = "AXIOM_LAUNCHER_PATH";
...
export function getDaemonRuntimeIdentity(environment: NodeJS.ProcessEnv = process.env): DaemonRuntimeIdentity {
	...
	const launcher = environment[AXIOM_LAUNCHER_PATH_ENV];
	return {
		buildId: environment[AXIOM_BUILD_ID_ENV] ?? bundledBuildId() ?? `release-${VERSION}`,
