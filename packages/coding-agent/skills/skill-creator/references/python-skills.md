...
A Python-backed skill is a regular markdown skill that also ships a Python package. Axiom installs the package editable into the kernel venv (`~/.axiom/agent/kernel-venv` by default, `AXIOM_KERNEL_VENV` to override) and exposes it in the persistent IPython kernel, so the agent can call it directly instead of shelling out.
...
- `rlm` and `tyro` are already present in the kernel venv. Do **not** declare `axiom-runtime` as a dependency: it is bundled with Axiom, not published on PyPI, so declaring it breaks installs outside the kernel venv. The CLI entry point only works where the runtime is installed, i.e. inside the kernel venv.
...
- Declare every third-party package `run()` imports in `dependencies` — `pyproject.toml` is the source of truth. The one exception is `axiom-runtime` (see above).
...
- If the user sets `AXIOM_KERNEL_PYTHON`, Axiom installs nothing — skills whose imports are missing there are disabled with a warning.
