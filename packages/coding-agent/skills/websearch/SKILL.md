...
Get a free API key at https://serper.dev, then run `/login` in Axiom,
switch to **MCP Connections**, and choose **Serper (web search)** to paste it.
The key is stored in Axiom and made available to this skill automatically.
...
- `AXIOM_WEBSEARCH_TIMEOUT` - HTTP timeout in seconds (default 45).
- `AXIOM_WEBSEARCH_NUM_RESULTS` - number of organic results to return (default 5).
