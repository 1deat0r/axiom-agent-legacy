...
import { AXIOM_TRACES_PROVIDER_ID, PRIME_INFERENCE_PROVIDER_ID } from "../src/core/prime-inference-auth.js";
...
describe("agent trace upload", () => {
	...
	beforeEach(() => {
		...
		originalTraceApiKey = process.env.AXIOM_TRACES_API_KEY;
		...
		delete process.env.AXIOM_TRACES_API_KEY;
	...
	afterEach(() => {
		...
		if (originalTraceApiKey === undefined) {
			...
			process.env.AXIOM_TRACES_API_KEY = originalTraceApiKey;
		}
	...
	it("does not upload when trace sharing is disabled", async () => {
		...
		const result = await uploadAgentTraceFile({
			...
			authStorage: AuthStorage.inMemory({
				[AXIOM_TRACES_PROVIDER_ID]: { type: "api_key", key: "trace-key" },
			}),
	...
	it("uses AXIOM_TRACES_BASE_URL for trace API overrides", async () => {
