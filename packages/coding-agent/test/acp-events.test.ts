...
describe("ACP session event mapping", () => {
	...
	it("gives bash a synthetic tool call with a stable id across its lifecycle", () => {
		...
		const mid = acpUpdatesForSessionEvent(
			fromPartial<AgentConnectionSessionEvent>({ type: "bash_output", chunk: "a.ts\n" }),
			state,
		);
	...
	it("surfaces compaction as metadata rather than distorting a standard update", () => {
		...
		expect(updates[0]?._meta).toMatchObject({
			[AXIOM_META_NAMESPACE]: { compaction: { tokensBefore: 1234, summary: "compacted" } },
		});
	});
	...
	it("surfaces continual-harness refinement outcomes, applied edits only", () => {
		...
		expect(failed[0]?._meta).toMatchObject({
			[AXIOM_META_NAMESPACE]: { refinement: { status: "failed", error: "budget exhausted" } },
		});
	});
	...
	it("streams bash output incrementally and surfaces compaction over ACP", () => {
		...
		const mid = acpUpdatesForSessionEvent(
			fromPartial<AgentConnectionSessionEvent>({ type: "bash_output", chunk: "hi\n" }),
		);
		const end = acpUpdatesForSessionEvent(
			fromPartial<AgentConnectionSessionEvent>({
				type: "bash_end",
		...
		expect(compaction[0]?._meta).toMatchObject({
			[AXIOM_META_NAMESPACE]: { compaction: { tokensBefore: 90_000, summary: "kept the last turns" } },
		});
	});
	...
	it("emits nothing for events ACP has no place for", () => {
		expect(acpUpdatesForSessionEvent(fromPartial<AgentConnectionSessionEvent>({ type: "agent_start" }))).toEqual([]);
		expect(
			acpUpdatesForSessionEvent(fromPartial<AgentConnectionSessionEvent>({ type: "recap_update", recap: "x" })),
		).toEqual([]);
	});
});
