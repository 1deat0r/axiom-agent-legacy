...
class FakeDaemonClient {
	...
	async connect(): Promise<void> {
		if (this.connected) {
			throw new Error("Axiom daemon client is already connected");
		}
		this.reconnectCount++;
	...
	disconnectForReconnect(reason: "shutdown" | "update"): void {
		this.closeCount++;
		this.connected = false;
		this.emitClose(new DaemonSocketClosedError("/tmp/axiom.sock", reason));
	}
}
...
describe("DaemonAgentConnection", () => {
	...
	it("reattaches when an update socket close arrives before the session notice", async () => {
		...
		fakeClient.emitClose(new DaemonSocketClosedError("/tmp/axiom.sock", "update"));
	...
	it("does not reconnect after an explicit shutdown session close", async () => {
		...
		expect(closedEvents[0]).toMatchObject({
			type: "closed",
			error: expect.stringContaining("The Axiom daemon shut down while this window was attached."),
		});
	...
	it("does not infer an update when shutdown closes the socket before the session notice", async () => {
		...
		fakeClient.emitClose(new DaemonSocketClosedError("/tmp/axiom.sock", "shutdown"));
		await Promise.resolve();

		expect(fakeClient.reconnectCount).toBe(0);
		expect(closedEvents).toHaveLength(1);
		const closedError = closedEvents[0]?.type === "closed" ? closedEvents[0].error : undefined;
		expect(closedError).toContain("The Axiom daemon shut down while this window was attached.");
	});
	...
	it("adds recovery and diagnostic context to unexpected daemon disconnects", async () => {
		...
		expect(closedEvents).toHaveLength(1);
		const closedError = closedEvents[0]?.type === "closed" ? closedEvents[0].error : undefined;
		expect(closedError).toContain("Lost connection to the Axiom daemon. Cause: ECONNRESET");
		expect(closedError).toContain("restart Axiom or reopen the session from Agents View");
		expect(closedError).toContain("Session file: /tmp/session-current.jsonl.");
		expect(closedError).toContain("Diagnostic log:");
	});
	...
	it("reports an unannounced clean socket close without treating it as an update", async () => {
		...
		fakeClient.emitClose(new DaemonSocketClosedError("/tmp/axiom.sock"));
		await Promise.resolve();

		expect(fakeClient.reconnectCount).toBe(0);
		expect(closedEvents).toHaveLength(1);
		const closedError = closedEvents[0]?.type === "closed" ? closedEvents[0].error : undefined;
		expect(closedError).toContain("Lost connection to the Axiom daemon.");
	});
	...
	it("returns to normal close handling after update restoration times out", async () => {
		...
		try {
			...
			expect(closedEvents).toHaveLength(1);
			const closedError = closedEvents[0]?.type === "closed" ? closedEvents[0].error : undefined;
			expect(closedError).toContain("The Axiom daemon restarted for an update, but this window could not reconnect");
			expect(closedError).toContain("Last error: daemon unavailable");
			expect(closedError).toContain("restart Axiom and reopen it from Agents View");
			expect(closedError).toContain("Session ID: session-current.");
			expect(closedError).toContain("Session file: /tmp/session-current.jsonl.");
			expect(closedError).toContain("Diagnostic log:");
