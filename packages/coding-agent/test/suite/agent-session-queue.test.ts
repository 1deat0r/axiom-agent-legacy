...
describe("AgentSession queue characterization", () => {
	...
	it.each([
		...
		async ({
			...
			const harness = await createAutoRefineHarness();
			harnesses.push(harness);
			const previousAgentDir = process.env.AXIOM_CODING_AGENT_DIR;
			process.env.AXIOM_CODING_AGENT_DIR = `${harness.tempDir}/agent`;
			try {
				...
				if (previousAgentDir === undefined) {
					...
					process.env.AXIOM_CODING_AGENT_DIR = previousAgentDir;
				}
			}
		},
	);
	...
	it("rolls back copied local refinement history against the original local harness state", async () => {
		...
		const previousAgentDir = process.env.AXIOM_CODING_AGENT_DIR;
		process.env.AXIOM_CODING_AGENT_DIR = `${original.tempDir}/agent`;
		try {
			...
			branched.sessionManager.appendCustomEntry("axiom.refinement", originalRefinement);
	...
	it("rolls back a local refinement in a non-persisted session via the recorded state path", async () => {
		...
		try {
			...
			harness.sessionManager.appendCustomEntry("axiom.refinement", seeded);
	...
	it("keeps a legacy scope-less rollback in the global store with global scope", async () => {
		...
		try {
			...
			harness.sessionManager.appendCustomEntry("axiom.refinement", legacyRefinement);
