...
describe("peers facade", () => {
	it("two instances see each other; a crashed one goes stale", () => {
		const s = scope();
		try {
			registerRun(s, A, { model: "m1", intent: "editing docs" }, { uuid: () => "run-a", pid: 101 });
			registerRun(s, B, { model: "m2" }, { uuid: () => "run-b", pid: 102 });
			registerRun(s, C, { model: "m3" }, { uuid: () => "run-c", pid: 103 });
	...
	it("directed messages reach only the target; group messages reach all", () => {
		...
		try {
			...
			registerRun(s, C, {}, { uuid: () => "run-c", pid: 103 });
			sendPeerMessage(s, A, "run-a", B.instanceId, "only for B", { now: () => NOW });

			expect(inbox(s, B, { now: () => NOW }).messages.map((m) => m.text)).toEqual(["only for B"]);
			expect(inbox(s, C, { now: () => NOW }).messages).toHaveLength(0);
			expect(inbox(s, A, { now: () => NOW }).messages).toHaveLength(0);

			sendPeerMessage(s, A, "run-a", "*", "everyone", { now: () => NOW });
			expect(inbox(s, B, { now: () => NOW }).messages.map((m) => m.text)).toEqual(["everyone"]);
			expect(inbox(s, C, { now: () => NOW }).messages.map((m) => m.text)).toEqual(["everyone"]);
		} finally {
			rmSync(s, { recursive: true, force: true });
		}
	});
	...
	it("inbox marks read; peekInbox does not", () => {
		...
		try {
			...
			expect(peekInbox(s, B, { now: () => NOW }).messages.map((m) => m.text)).toEqual(["hello"]);
			expect(peekInbox(s, B, { now: () => NOW }).messages.map((m) => m.text)).toEqual(["hello"]);
			const read = inbox(s, B, { now: () => NOW });
			expect(read.messages.map((m) => m.text)).toEqual(["hello"]);
			expect(inbox(s, B, { now: () => NOW }).messages).toHaveLength(0);
		} finally {
			rmSync(s, { recursive: true, force: true });
		}
	});
	...
	it("intent updates are visible to peers and heartbeat keeps a run live", () => {
		...
		try {
			...
			expect(listPeers(s, B, { now: () => NOW, staleMs: DEFAULT_STALE_MS, pidAlive: () => true }).active[0]?.intent).toBe(
				"on branch feat/x",
			);
			expect(heartbeatRun(s, "run-a", { now: () => NOW + 60_000 })).toBe(true);
			expect(
				listPeers(s, B, { now: () => NOW + 60_000, staleMs: DEFAULT_STALE_MS, pidAlive: () => true }).active[0]?.status,
			).toBe("active");
		} finally {
			rmSync(s, { recursive: true, force: true });
		}
