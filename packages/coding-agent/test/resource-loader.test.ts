...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".axiom", "agent", "prompts");
			...
			const projectSkillDir = join(cwd, ".axiom", "agent", "skills", "collision-skill");
...
Project skill`,
			...
			const baseTheme = fromPartial<{ name: string; vars?: Record<string, string> }>(
				JSON.parse(readFileSync(join(process.cwd(), "src", "modes", "interactive", "theme", "dark.json"), "utf-8")),
			);
			baseTheme.name = "collision-theme";
			const userThemePath = join(agentDir, "themes", "collision.json");
			const projectThemePath = join(cwd, ".axiom", "agent", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".axiom", "agent", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".axiom", "agent"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".axiom", "agent", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".axiom", "agent", "extensions", "shared.ts"));
		});

		it("should keep both extensions loaded when command names collide", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".axiom", "agent", "extensions");
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.pi", async () => {
			const piDir = join(cwd, ".axiom", "agent");
...
Content`,
	...
	describe("bundled skills", () => {
		...
		it("should let a project skill override the bundled websearch skill", async () => {
			const projectSkillDir = join(cwd, ".axiom", "agent", "skills", "websearch");
			mkdirSync(projectSkillDir, { recursive: true });
			writeFileSync(
