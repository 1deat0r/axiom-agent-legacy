...
describe("kernel bootstrap", () => {
	beforeEach(async () => {
		...
		tempDir = mkdtempSync(join(tmpdir(), "axiom-kernel-bootstrap-"));
		process.env.HOME = tempDir;
		process.env.PATH = originalEnv.PATH ?? "";
		delete process.env.AXIOM_KERNEL_PYTHON;
		delete process.env.AXIOM_KERNEL_VENV;
	...
	it("bootstraps a missing venv with uv, ipykernel, axiom-runtime, and default extra packages", async () => {
		...
		expect(log).toContain("axiom-runtime");
	...
	it("installs sibling Python skill dependencies when package and directory names differ", async () => {
		...
		writeFileSync(
			...
			`[project]
name = "axiom-skill-attach-image"
version = "0.1.0"
`,
		);
		const dependentSkill = createPythonSkillWithDependency("orchestration-heartbeat", "axiom-skill-attach-image");
		process.env.AXIOM_KERNEL_VENV = venv;
...
`,
	...
	it("uses AXIOM_KERNEL_PYTHON as an override contract", async () => {
		...
		process.env.AXIOM_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).resolves.toBe(overridePython);
	});

	it("allows AXIOM_KERNEL_PYTHON missing Python skill imports", async () => {
		const overridePython = join(tempDir, "override-python");
		const pythonSkill = createPythonSkill();
		writeFakePython(overridePython, ["ipykernel", "rlm", ...DEFAULT_RLM_EXTRA_IMPORT_NAMES]);
		process.env.AXIOM_KERNEL_PYTHON = overridePython;
	...
	it("rejects AXIOM_KERNEL_PYTHON missing default extra packages", async () => {
		const overridePython = join(tempDir, "override-python");
	...
	it("rejects AXIOM_KERNEL_PYTHON with a stale rlm runtime", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, ["ipykernel"]);
		process.env.AXIOM_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).rejects.toThrow(/current axiom-runtime with callable rlm\.run/);
	});

	it("rejects AXIOM_KERNEL_PYTHON with a legacy harness API", async () => {
		const overridePython = join(tempDir, "override-python");
		writeExecutable(
			overridePython,
			[
	...
	it("fails an invalid AXIOM_KERNEL_PYTHON without bootstrapping", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, []);
		process.env.AXIOM_KERNEL_PYTHON = overridePython;
