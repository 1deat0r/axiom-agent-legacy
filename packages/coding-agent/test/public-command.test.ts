...
vi.mock("../src/package-manager-cli.js", () => ({
	...
	isSelfUpdateSource: (source: string) => source === "self" || source === "pi" || source === "axiom",
}));
...
describe("public command routing", () => {
	...
	it("rejects extra attach operands", async () => {
		...
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("axiom attach <agent>"));
	});
	...
	it("separates Axiom updates from package updates", async () => {
	...
	it("gives legacy update targets explicit migration guidance", async () => {
		for (const target of ["self", "--self", "axiom"]) {
			await handlePublicCommand(["update", target]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "axiom update [--force]"'));
	});
	...
	it("directs legacy package-update forms to the package command", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "axiom package update [source]"'));
	});
	...
	it("rejects self-update aliases on the package update path", async () => {
		for (const source of ["self", "pi", "axiom"]) {
			await handlePublicCommand(["package", "update", source]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "axiom update"'));
	});
	...
	it("directs package uninstall to package remove", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "axiom package remove"'));
	...
	it("rejects operands for package list", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("axiom package list"));
	});
	...
	it("rejects the old daemon hierarchy with migration guidance", async () => {
		...
		expect(process.exitCode).toBe(1);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Run "axiom help"'));
	});

	it("shows migration guidance when help targets removed commands", async () => {
		const cases: Array<[path: string[], hint: string]> = [
			[["daemon"], 'Run "axiom help"'],
			[["install"], 'Use "axiom package install"'],
			[["remove"], 'Use "axiom package remove"'],
			[["uninstall"], 'Use "axiom package remove"'],
			[["manage"], 'Use "axiom agents"'],
			[["app", "update"], 'Use "axiom update"'],
		];
	...
	it("shows command help when options precede the help flag", async () => {
		...
		expect(console.log).toHaveBeenNthCalledWith(1, expect.stringContaining("axiom list [--all] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(2, expect.stringContaining("axiom doctor [--fix] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(3, expect.stringContaining("axiom package install <source>"));
