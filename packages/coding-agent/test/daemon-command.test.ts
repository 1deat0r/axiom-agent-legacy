...
describe("daemon command", () => {
	...
	it("cleans prompt listeners when the prompt request fails", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "prompt", "active-1", "hello"]),
		).resolves.toBe(true);
	...
	it("ignores stale agent_end events before a daemon prompt starts", async () => {
		...
		const command = handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "prompt", "active-1", "hello"]);
	...
	it("ends json attach when the session closes", async () => {
		const command = handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "--json", "attach", "active-1"]);
	...
	it("chooses a terminating non-colliding default name past the safe-integer range", async () => {
		...
		await expect(handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock"])).resolves.toBe(true);
	...
	it("keeps create session name after an unknown boolean extension flag", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "create", "--unknown-typo", "my-session"]),
		).resolves.toBe(true);
	...
	it("parses extension flag values with equals without consuming the create name", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "create", "--ticket=123", "my-session"]),
		).resolves.toBe(true);
	...
	it("keeps bare --resume values as session id selectors", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "create", "--resume", "abc123"]),
		).resolves.toBe(true);
	...
	it("rejects unknown send options instead of folding them into the message", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "send", "worker", "--bogus", "hello"]),
		).resolves.toBe(true);
	...
	it("rejects extra agent-messages status arguments", async () => {
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "agent-messages", "pause", "active-1"]),
		).resolves.toBe(true);
	...
	it("resolves agent names before filtering scheduled prompts", async () => {
		...
		await expect(
			handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "--json", "cron", "list", "alpha"]),
		).resolves.toBe(true);
	...
	it("rejects empty --goal in daemon create", async () => {
		await handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "create", "--goal", "  ", "my-session"]);
	...
	it("does not leak goal into default config for a subsequent no-goal create", async () => {
		...
		await handleDaemonCommand(["daemon", "--socket", "/tmp/axiom.sock", "create", "second"]);
