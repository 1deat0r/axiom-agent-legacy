...
describe("parseSsListeners", () => {
	const stdout = [
		...
		'u_str LISTEN 0      511    /tmp/custom.sock 10147608 * 0 users:(("axiom",pid=1234,fd=22))',
		'u_str LISTEN 0      511    /tmp/axiom-1000/daemon.sock 79453846 * 0 users:(("axiom",pid=5678,fd=24))',
		'u_str LISTEN 0      4096   /run/dbus/system_bus_socket 123 * 0 users:(("dbus-daemon",pid=900,fd=3))',
		'u_str ESTAB  0      0      /tmp/other.sock 456 * 0 users:(("axiom",pid=4321,fd=9))',
		"",
	].join("\n");

	it("extracts socket + pid for axiom LISTEN sockets only", () => {
		const daemons = parseSsListeners(stdout, "axiom");
		expect(daemons).toEqual([
			{ pid: 1234, socketPath: "/tmp/custom.sock" },
			{ pid: 5678, socketPath: "/tmp/axiom-1000/daemon.sock" },
		]);
	});
...
describe("parsePrimeAgentProcessIds", () => {
	it("finds process.title names even when lsof reports the executable as node", () => {
		const stdout = [
			"  123 node axiom --mode daemon",
			"  456 axiom axiom",
			"  789 /usr/local/bin/axiom axiom",
			"  900 node unrelated.js",
			"",
		].join("\n");
		expect(parsePrimeAgentProcessIds(stdout, "axiom")).toEqual([123, 456, 789]);
	});
});
...
describe("planReap", () => {
	...
	it("only kills unreachable daemons with --force", () => {
		...
		expect(skipped.kind === "skip" ? skipped.reason : "").toContain("axiom shutdown --force");
