...
function emitHello(
	...
	socket.emit(
		...
		`${JSON.stringify({
			type: "daemon_hello",
			socketPath: "/tmp/axiom.sock",
			protocol: { name: "axiom.daemon", version },
			schemaRevision,
			appVersion: "9.9.9",
			clientId: "client-1",
			serverCapabilities,
...
describe("DaemonClient", () => {
	...
	it("allows connect retry after the socket emits an error before connecting", async () => {
		const client = new DaemonClient("/tmp/axiom-missing.sock");
		...
		expect(firstError.message).toContain("Failed to connect to the Axiom daemon: initial connect failed.");
		expect(firstError.message).toContain("Socket: /tmp/axiom-missing.sock.");
		...
		await expect(secondAttempt).resolves.toMatchObject({
			message: expect.stringContaining("Failed to connect to the Axiom daemon: retry reached socket."),
		});
	});

	it("allows connect retry after the initial connection times out", async () => {
		vi.useFakeTimers();
		const client = new DaemonClient("/tmp/axiom-slow.sock");
		...
		const timeoutRejection = expect(firstAttempt).resolves.toMatchObject({
			message: expect.stringContaining("Timed out after 5ms connecting to the Axiom daemon."),
		});
	...
	it("captures the daemon hello greeting for version checks", async () => {
		const client = new DaemonClient("/tmp/axiom.sock");
		...
		const hello = {
			type: "daemon_hello",
			socketPath: "/tmp/axiom.sock",
			protocol: { name: "axiom.daemon", version: 1 },
			appVersion: "9.9.9",
			clientId: "client-1",
			serverCapabilities: [],
		};
	...
	it("serializes activeSessionId for session commands", async () => {
		...
		expect(envelope).toMatchObject({
			...
			protocol: { name: "axiom.daemon", version: DAEMON_PROTOCOL_VERSION },
	...
	it("includes command, socket, and log context when a request is made while disconnected", async () => {
		...
		await expect(request).rejects.toMatchObject({
			message: expect.stringContaining(
				'Cannot send daemon command "list" because the Axiom daemon is not connected.',
			),
		});
		await expect(request).rejects.toMatchObject({
			message: expect.stringContaining("Socket: /tmp/axiom.sock."),
		});
		await expect(request).rejects.toMatchObject({
	...
	it("notifies listeners when a connected daemon socket closes", async () => {
		...
		expect(closed[0]?.message).toContain("Connection to the Axiom daemon closed.");
		expect(closed[0]?.message).toContain("Socket: /tmp/axiom.sock.");
