...
function createHomebrewInstall(): { packageDir: string } {
	...
	const packageDir = join(prefix, "Cellar", "axiom", "0.7.0", "libexec", "lib", "node_modules", "axiom");
	mkdirSync(packageDir, { recursive: true });
	tempDir = prefix;
	process.env.PI_PACKAGE_DIR = packageDir;
	setExecPath(join(packageDir, "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	...
	test("leaves Homebrew installs under Homebrew ownership", () => {
		...
		expect(getSelfUpdateCommand("axiom")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("axiom")).toBe("Update with: brew upgrade axiom");
		expect(getUpdateInstruction("axiom")).toBe("Update with: brew upgrade axiom");
	});
	...
	test("self-updates tarball specs without uninstalling the same logical package first", () => {
		const { prefix } = createNpmPrefixInstall();
		const tarballUrl = "https://downloads.example.test/axiom/axiom-0.73.0.tgz";
	...
	test("self-updates renamed tarball packages by uninstalling the old package after install", () => {
		...
		const command = getSelfUpdateCommand("@earendil-works/pi-coding-agent", undefined, tarballUrl, "axiom");
...
describe("session paths", () => {
	test("uses the short app-prefixed session dir env var", () => {
		expect(ENV_SESSION_DIR).toBe("AXIOM_SESSION_DIR");
	});
	...
	test("expands tilde in the session root env var", () => {
		process.env[ENV_SESSION_DIR] = "~/axiom-sessions";

		expect(getSessionsDir("/agent")).toBe(join(homedir(), "axiom-sessions"));
	});
