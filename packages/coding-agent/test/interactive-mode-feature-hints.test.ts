...
describe("feature hint deck", () => {
	...
	it("covers Axiom workflows with capability-focused copy", () => {
		const deck = new FeatureHintDeck(() => 0);
		...
		expect(textById.get("subagents")).toBe("Axiom can delegate tasks to subagents and run them in parallel.");
