...
function socketPathForRun(): string {
	...
	if (process.platform === "win32") {
		return `\\\\.\\pipe\\axiom-multiclient-bench-${process.pid}-${socketCounter}`;
	}
	return `/tmp/axiom-multiclient-${process.pid}-${socketCounter}.sock`;
}
...
const streamCorpus = repeatedText(STREAM_CHARS, "Axiom daemon multi-client streaming benchmark. ");
...
function createAttachResponseForMessages(
	...
	const state = {
		...
		sessionDir: "/tmp/axiom-benchmark/sessions",
		...
		for (let offset = 0, sequence = 1; offset < streamCorpus.length; sequence++) {
			...
			const compact = createCompactAssistantDelta({
				...
				meta: {
					...
					protocol: { name: "axiom.daemon", version: 2 },
					activeSessionId: "active-benchmark",
					sequence,
...
async function runAttachV2(
	...
	const cacheRoot = await mkdtemp(join(tmpdir(), "axiom-attach-v2-bench-"));
...
async function loadSessionFixture(sessionFile: string): Promise<{
	...
	const tempDir = await mkdtemp(join(tmpdir(), "axiom-session-bench-"));
...
function printResults(
	...
	console.log("Axiom daemon multi-client benchmark");
	console.log(details);
	console.table(
		results.map((result) => ({
...
function printSessionLoad(sessionFile: string, result: SessionLoadResult): void {
	console.log("Axiom real-session benchmark");
...
async function generateSessionFixture(sizeMiB: number): Promise<GeneratedSessionFixture> {
	const directory = await mkdtemp(join(tmpdir(), `axiom-generated-${sizeMiB}mib-`));
