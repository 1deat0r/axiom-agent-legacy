...
import axiomLedgerExtension, { createLedgerExtension, defaultLedgerDeps } from "../../src/extensions/ledger/index.js";
import {
	addTotals,
	aggregateUsage,
	applyOverrides,
	...
	type OverrideRates,
	priceUsage,
} from "../../src/extensions/ledger/ledger.js";
import { loadOverrides } from "../../src/extensions/ledger/storage.js";
...
describe("aggregateUsage", () => {
	...
	it("skips user entries without usage", () => {
		const buckets = aggregateUsage([userEntry()]);
		expect(buckets).toEqual([]);
	});
...
describe("priceUsage", () => {
	...
	it("prices all cache writes at the cacheWrite rate (no 1h tier in v0.7.2)", () => {
		const u = usage({ cacheWrite: 10_000 });
		const cost = priceUsage(u, rates);
		expect(cost.cacheWrite).toBeCloseTo(0.01, 12);
		expect(cost.total).toBeCloseTo(0.01, 12);
	});
...
describe("createLedgerExtension", () => {
	it("registers the /cost command and an agent_end handler", () => {
	...
	it("agent_end sets a live cost status", async () => {
