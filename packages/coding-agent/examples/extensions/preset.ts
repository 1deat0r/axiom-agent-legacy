 ...
 * - ~/.axiom/agent/presets.json (global)
 * - <cwd>/.axiom/agent/presets.json (project-local)
 *
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".axiom", "agent", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.axiom/agent/presets.json", "warning");
			return;
		}
