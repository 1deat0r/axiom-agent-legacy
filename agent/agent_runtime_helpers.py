...
def agent_runtime_owns_post_tool_hook(agent: Any, function_name: str) -> bool:
    ...
    ADR-0090: ownership derives from the registry seam — a tool with a
    registered ``agent_executor`` emits its post hook from the executor path.
    """
    try:
        from tools.registry import registry

        if registry.get_agent_executor(function_name) is not None:
            return True
    except Exception:
        pass
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    # ADR-0090: one dispatch seam — agent-level tools resolve their
    # agent_executor from the registry instead of a name-forks chain. The
