...
| 8 | **Profiles** — `--profile` separate homes, SOUL.md identity, never two processes | ADR-0014 | agent dirs exist per-home, but no `--profile` boot | **SHIPPED** (2026-08-12, 185bec2): `--profile` pre-scan sets AXIOM_HOME + `AXIOM_CODING_AGENT_DIR`; `axiom-profile` extension |
...
- Env rename `PI_CODING_AGENT_DIR` -> `AXIOM_CODING_AGENT_DIR`
