# ADR-0015: prime-agent v0.7.2 becomes the baseline
...
and now, the same author (Mario Zechner) shipped **prime-agent v0.7.2** (the
project's successor to pi), and the owner directed a complete restart:
**prime-agent v0.7.2 becomes axiom-agent's base starting point.**

Facts verified 2026-08-12:

- prime-agent is `github.com/PrimeIntellect-ai/prime-agent`, MIT, npm-workspaces
...
- Latest release: **v0.7.2**, tag `83a0f9f`. The `coding-agent` package is the
  agent CLI (config `piConfig.name = "prime-agent"`, configDir `.prime/agent`,
...
- The extension seam axiom ports ride is 1:1 against the pi baseline except:
  ...
  `PRIME_AGENT_CODING_AGENT_DIR` (`ENV_AGENT_DIR` from config.ts).
- prime-agent HAS: `/usage` cost+context display (per-session), a `/goal`
...
- prime-agent does NOT have: a lifetime cost ledger with override repricing, a
...
1. **The axiom-agent repo becomes a hard fork of prime-agent v0.7.2** on
   branch `baseline/prime-v0.7.2`. `upstream` points at
   PrimeIntellect-ai/prime-agent; upstream merges are routine
...
3. **Axiom capabilities stay axiom-only** and are re-ported onto the v0.7.2
   ...
   capabilities (ADR-0010/0011/0008/0014). What prime-agent now covers
...
4. **Boot seam:** `--profile <name>` is pre-scanned in `main()` before any
   ...
   (`PRIME_AGENT_CODING_AGENT_DIR`) to the profile home. A decrypt-regression
...
5. **Data cutover:** axiom-owned durable data (`~/.axiom/ledger.json`
   ...
   (`.prime/agent`); pi-era sessions are unmigrated and readable on the
