"""Tiny rlm-compatible kernel shim for Axiom."""
...
async def host_request(request_type: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    """Send a typed request to the Axiom host and await its reply.
...
async def run(prompt: str, **kwargs: Any) -> RLMSpawnHandle:
    """Spawn a recursive Axiom child and return once its task is admitted.
