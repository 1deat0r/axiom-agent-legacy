<h3 align="center">
Axiom: A Self-Improving RLM Agent
</h3>

<p align="center">
  <a href="packages/coding-agent/docs/index.md">Documentation</a> &bull;
  <a href="https://github.com/mustbearnold/axiom-agent/actions/workflows/ci.yml">
    <img src="https://github.com/mustbearnold/axiom-agent/actions/workflows/ci.yml/badge.svg" alt="CI" />
  </a>
  <a href="https://github.com/mustbearnold/axiom-agent/actions/workflows/build-binaries.yml">
    <img src="https://github.com/mustbearnold/axiom-agent/actions/workflows/build-binaries.yml/badge.svg" alt="Build Binaries" />
  </a>
</p>
...
Axiom combines a persistent Python control environment with durable harness state, so useful working context and reusable operating patterns can outlive a single chat window.
...
Run it from the repository or directory you want it to work in:
...
The `axiom` command is installed by the self-deploy script (see `~/.axiom/axiom-self-deploy.sh`) or your preferred packaging path. The installer verifies the release checksum, installs the `axiom` command, and can prepare the IPython runtime used by the agent.

On first launch, run `/login` to choose a subscription or API-key provider. Axiom works in the current directory and can run commands and modify files there. Use a disposable clone, clean worktree, or another checkpoint you can inspect and restore.

> [!WARNING]
> Axiom executes model-generated Python and project commands with your user permissions. Its worker and kernel processes improve lifecycle isolation and recovery; they are **not** a security sandbox. Review changes and use trusted repositories, instructions, skills, and extensions only. Run untrusted code or instructions in an external sandbox or restricted environment.

Useful commands:

```bash
axiom agents                   # Browse running, idle, and saved sessions
axiom attach <agent>           # Reattach to a running session
axiom --resume <path|id>       # Resume a saved session
axiom status                   # Inspect background service state
axiom doctor [--fix]           # Inspect or repair background services
axiom update [--force]         # Update Axiom
axiom shutdown [--force]       # Stop every agent, worker, and background service
```

## Built for Long-Running Work
Axiom is built for long-running work, especially for evaluations in research. These features are available in the TUI, and when run autonomously.
...
- **Heartbeats and schedules:** `/heartbeat`, `rlm_heartbeat`, and `axiom schedule` can re-enter a session periodically or at a specific time.
...
Axiom ships a cross-platform messaging gateway (Telegram, Discord, Slack) so you can drive the agent and its automation from chat. Gateway commands — `/search`, `/sessions`, `/cron`, `/announce`, `/ledger`, `/profiles`, `/projects`, `/soul`, `/help` — never reach the model and are handled locally.
...
Axiom is a fork of **Prime Agent** (MIT) and shares its RLM/continual-harness lineage from [`pi`](https://github.com/earendil-works/pi). We thank the Prime Intellect and `pi` authors for their valuable work.

## License

Axiom is fully open source and released under the [MIT License](LICENSE).
