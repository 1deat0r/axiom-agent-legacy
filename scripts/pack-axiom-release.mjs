...
// TODO: Remove this R2 tarball packer once axiom and its internal workspace
...
const defaultBaseUrl = process.env.AXIOM_DOWNLOAD_BASE_URL;
const publicPackageName = process.env.AXIOM_PACKAGE_NAME || "axiom";
const publicCommandName = process.env.AXIOM_CMD || "axiom";
const releaseChannels = new Set(["stable", "beta"]);

const releasePackages = [
	{ packageDir: "ai", publicName: undefined, artifactName: "axiom-ai" },
	{ packageDir: "tui", publicName: undefined, artifactName: "axiom-tui" },
	{ packageDir: "agent", publicName: undefined, artifactName: "axiom-core" },
...
function parseArgs(args) {
	...
	if (!parsed.baseUrl) {
		throw new Error("--base-url or AXIOM_DOWNLOAD_BASE_URL is required");
	}
...
function printHelp() {
	console.log(`Usage: node scripts/pack-axiom-release.mjs --base-url url [--channel stable|beta] [--version x.y.z] [--out-dir path]

Creates private npm tarballs for R2 distribution:

  <out-dir>/artifacts/axiom-<version>.tgz
  <out-dir>/artifacts/axiom-ai-<version>.tgz
  <out-dir>/artifacts/axiom-core-<version>.tgz
  <out-dir>/artifacts/axiom-tui-<version>.tgz
...
function main() {
	...
	const releaseVersion = args.version || normalizeVersion(process.env.AXIOM_VERSION || cliPackage.version);
